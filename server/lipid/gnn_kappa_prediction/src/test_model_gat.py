...
class GATPredictor(torch.nn.Module):
    def __init__(self, input_features, hidden_features):
        ...
        self.conv1 = GATConv(input_features, hidden_features)  # First GAT layer
        self.conv2 = GATConv(hidden_features, hidden_features)  # Second GAT layer
        self.fc = torch.nn.Linear(hidden_features, 1)  # Linear layer for output

    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        x = F.relu(self.conv1(x, edge_index))  # Apply first GAT layer and ReLU activation
        x = F.relu(self.conv2(x, edge_index))  # Apply second GAT layer and ReLU activation
...
def train_model():
    ...
    model = GATPredictor(input_features=9, hidden_features=32)
